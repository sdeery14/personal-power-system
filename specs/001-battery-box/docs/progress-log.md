...
- **Simplified panel architecture**: Left side plywood panel only (~12"×13" for Blue Sea 5026 + Victron shunt - heavy components need rigid backing). Displays mount directly to right side HDPE (Nilight + Victron BMV-712 lightweight, grooves help Nilight fit flush). KarlKers connectors on front/back short side HDPE. Benefits: one plywood panel instead of two (simpler), displays positioned where tote grooves provide better support, RJ12 cable routes across battery top from shunt (left) to display (right).
